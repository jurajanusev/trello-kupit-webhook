...
@app.route("/api/create-riverdale-workflow-pilot", methods=["POST"])
def create_riverdale_workflow_pilot():
    if request.headers.get("X-Pilot-Key") != "riverdale-pilot-4f91c7a62d":
        return jsonify({"error": "forbidden"}), 403

    board = trello_get("/boards/CzuD55PR", {"fields": "name,url"})
    lists = trello_get(f"/boards/{board['id']}/lists", {"fields": "name,closed"})
    pilot_list = next((item for item in lists if not item.get("closed") and item.get("name") == "PILOT — WORKFLOW"), None)
    if not pilot_list:
        pilot_list = trello_post_body("/lists", {"idBoard": board["id"], "name": "PILOT — WORKFLOW", "pos": "bottom"})

    wanted_labels = {
        "MOBIL": "yellow",
        "NADVÄZNOSŤ": "red",
        "SCREEN": "purple",
        "VYROBIŤ / KÚPIŤ": "orange",
        "PILOT": "sky",
    }
    board_labels = trello_get(f"/boards/{board['id']}/labels", {"fields": "name,color", "limit": 1000})
    labels_by_name = {item.get("name", "").strip().casefold(): item for item in board_labels}
    for label_name, color in wanted_labels.items():
        if label_name.casefold() not in labels_by_name:
            label = trello_post_body("/labels", {"idBoard": board["id"], "name": label_name, "color": color})
            labels_by_name[label_name.casefold()] = label

    existing_cards = trello_get(f"/lists/{pilot_list['id']}/cards", {"fields": "name", "limit": 100})
    if existing_cards:
        return jsonify({"status": "exists", "list": pilot_list["name"], "cards": existing_cards})

    def label_ids(*names):
        return ",".join(labels_by_name[name.casefold()]["id"] for name in names)
    ...
    scene = trello_post_body("/cards", {
        "idList": pilot_list["id"],
        "name": "[VZOR] 03/28. INT. ŠKOLA — CHLAPČENSKÁ ŠATŇA, DEŇ — BETY, VERONIKA, KIKO, EVA, SÁRA",
        "desc": (
            "**TYP:** Obraz\n\n"
            "**LOKÁCIA:** Škola — chlapčenská šatňa\n"
            "**NATÁČANIE:** ukážkový dátum 31. 7. 2026\n"
            "**PORADIE DŇA:** 4\n\n"
            "#### Odhalenie tajného kanála\n\n"
            "Bety, Veronika, Eva a Kiko prehľadávajú skrinky basketbalistov. Sebov mobil odomknú PIN-om 5656 a nájdu kanál Blackstone&sluts.\n\n"
            "**KONTINUITA:** Sebov mobil musí mať rovnaký čierny obal a pripravený obsah obrazovky."
        ),
        "idLabels": label_ids("PILOT", "MOBIL", "NADVÄZNOSŤ", "SCREEN"),
        "pos": "bottom",
    })
    add_checklist(scene["id"], "REKVIZITY", [
        "Sebov mobil — čierny obal; Bety ho odomkne PIN-om 5656 a otvorí tajný kanál Blackstone&sluts",
        "Tímová selfie basketbalistov — visí pri dverách a napovie Bety Sebov PIN",
    ])
    add_checklist(scene["id"], "PRIPRAVENOSŤ", [
        "Mobil fyzicky pripravený a nabitý",
        "Screeny otestované na zariadení",
        "Kontinuitná fotografia skontrolovaná",
        "Náhradný mobil / offline video pripravené",
    ])
    add_checklist(scene["id"], "Info z natáčania", [
        "Po obraze doplniť skutočný stav mobilu a fotografiu kontinuity",
    ])

    prop = trello_post_body("/cards", {
        "idList": pilot_list["id"],
        "name": "[VZOR] SEBO — mobil",
        "desc": (
            "**TYP:** Centrálna rekvizita\n\n"
            "**VLASTNÍK:** Sebo\n"
            "**VARIANT:** čierny mobil s čiernym obalom\n"
            "**SKLAD:** doplniť\n"
            "**STAV:** pripravuje sa\n\n"
            "Hlavný zdroj pravdy pre fyzický vzhľad, screeny, poškodenie a kontinuitu mobilu.\n\n"
            "**POSLEDNÝ STAV:** pred 03/28 bez poškodenia; batéria na screene 63 %."
        ),
        "idLabels": label_ids("PILOT", "MOBIL", "NADVÄZNOSŤ"),
        "pos": "bottom",
    })
    add_checklist(prop["id"], "Výskyt v obrazoch", [
        "03/28 — Bety vyberie mobil zo skrinky a odomkne ho PIN-om 5656",
        "03/29 — zachovať rovnaký obal, screen a stav batérie",
    ])
    add_checklist(prop["id"], "Kontinuita", [
        "Nahrať referenčnú fotografiu spredu a zozadu",
        "Po obraze zapísať poškodenie, stav obrazovky a miesto uloženia",
    ])

    task = trello_post_body("/cards", {
        "idList": pilot_list["id"],
        "name": "[VZOR][SCREEN] Pripraviť Blackstone&sluts do Sebovho mobilu",
        "desc": (
            "**TYP:** Obstarávacia úloha\n\n"
            "**PRVÝ OBRAZ:** 03/28\n"
            "**NATÁČANIE:** 31. 7. 2026\n"
            "**INTERNÝ DEADLINE:** 29. 7. 2026\n\n"
            "Pripraviť tajný kanál, fotky, komentáre, lajky a správny PIN. Výstup musí fungovať aj bez internetu."
        ),
        "idLabels": label_ids("PILOT", "SCREEN", "VYROBIŤ / KÚPIŤ"),
        "due": "2026-07-29T12:00:00.000Z",
        "pos": "bottom",
    })
    add_checklist(task["id"], "POSTUP", [
        "Pripraviť grafiku kanála a obsah správ",
        "Nahrať obsah do fyzického mobilu",
        "Otestovať offline režim a jas displeja",
        "Schváliť s vedúcim rekvizít",
        "Pripraviť náhradné video / druhý mobil",
    ])

    daily = trello_post_body("/cards", {
        "idList": pilot_list["id"],
        "name": "[VZOR] REKVIZITY — NATÁČACÍ DEŇ 31. 7. 2026",
        "desc": (
            "**TYP:** Denná príprava\n\n"
            "Automaticky generovaný súhrn rekvizít podľa poradia obrazov. Rekvizitár používa túto kartu na balenie, výdaj a návrat."
        ),
        "idLabels": label_ids("PILOT"),
        "due": "2026-07-31T05:00:00.000Z",
        "pos": "bottom",
    })
    add_checklist(daily["id"], "OBRAZ 03/28 — ŠATŇA", [
        "Sebov mobil — čierny obal, screen Blackstone&sluts, PIN 5656",
        "Tímová selfie basketbalistov — osadiť pri dverách",
        "Kontinuita — skontrolovať referenčnú fotografiu pred prvou klapkou",
    ])
    add_checklist(daily["id"], "VÝDAJ A NÁVRAT", [
        "Prevzaté zo skladu",
        "Odovzdané na pľac",
        "Po obraze nafotené",
        "Vrátené do skladu / nahlásený problém",
    ])

    links = [
        (scene, prop, "Centrálna rekvizita — Sebov mobil"),
        (scene, task, "Úloha — pripraviť screen"),
        (scene, daily, "Denná príprava"),
        (prop, scene, "Obraz 03/28"),
        (prop, task, "Screenová úloha"),
        (task, scene, "Zdrojový obraz 03/28"),
        (task, prop, "Centrálna rekvizita"),
        (daily, scene, "Zdrojový obraz 03/28"),
        (daily, prop, "Kontinuita — Sebov mobil"),
    ]
    for source, target, name in links:
        trello_post_body(f"/cards/{source['id']}/attachments", {"url": target["shortUrl"], "name": name})

    trello_post_body(f"/cards/{task['id']}/actions/comments", {
        "text": "[VZOR] Automatický záznam: úloha vytvorená zo scenára, interný deadline je dva dni pred natáčaním."
    })

    return jsonify({
        "status": "created",
        "board": board["name"],
        "list": pilot_list["name"],
        "cards": [
            {"name": item["name"], "url": item["shortUrl"]}
            for item in (scene, prop, task, daily)
        ],
    })
